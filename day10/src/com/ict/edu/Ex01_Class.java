...
//						      기능 = 동작 = 멤버 메서드
//							 생성자 = 클래스가 객체로 만들어질때 한번 호출돼서 데이터를 초기화 할 수 있는 것
//						** 클래스이름 = 저장이름 = 파일이름 = 생성자이름 같다 **
	
//	클래스 구조: 헤더, 멤버필드, 생성자, 멤버메서드
//	1. 헤더: [접근 제한자], [클래스 종료에 대한 예약어], class 클래스이름{ }
//		1) 접근제한자: 클래스, 멤버필드, 멤버메서드, 생성자 모두 사용할 수 있다.
//					=> 외부에서 접근할 수있는 권한의 단계를 말함.
//			1-가) public :  누구나 다 접근 가능
//			1-나) protected : 같은 패키지와 상속관계에서는 접근 가능
//			1-다) 생략(default) : 같은 패키지에서만 접근이 가능
//			1-라) private : 외부에서는 접근 불가, 내부멤버(멤버필드, 멤버메서드)끼리만 접근 가능
...
//		4) 클래스이름: 클래스이름 = 저장이름 = 파일이름 = 생성자이름 같다
//					  첫글자 대문자, 나머지 소문자
//					  두단어 이상일때는 단어 첫글자들은 대문자
//					  숫자는 중간이다 뒤에 사용, 공백은 사용 불가
	
//	2. 멤버필드 : 데이터, 상태값, 속성
//		1)변수: 언제든지 변할수 있는 데이터를 저장하는 공간
//				첫글자 소문자, 두단어 이상일때는 단어 첫글자들은 대문자
//	  			숫자는 중간이다 뒤에 사용, 공백은 사용 불가
//		2)상수: 한번 데이터가 저장되면 변경할 수 없는 데이터를 저장하는 공간
//				모든 글자가 대문자, 단어와 단어 사이에 '_'(언더바)를 사용할 수 있다.
	
//	3. ★ 멤버메서드: 동작, 기능, 실행 ★
//		- 해당 메서드를 호출(실행)하면 해당 메소드 안에 존재한 내용이 실행됨
//		- 호출해서 내용이 실행 된 후에는 호출한 곳으로 되돌아 간다.
//		- 메서드는 다른 메서드를 호출할 수 있음.
//		1) 구성 : [접근제한자] [메서드종류] 반환형 메서드이름([인자=매개변수]) {  실행할 내용  }
//			1-가) 접근제한자: public, protected, 생략, private ;
//			1-나) 메서드종류: 일반메서드 이부분을 생략한다.
//							  특정 메서드에 해당하는 예약어를 사용 (static)
//	★중요★	1-다) 반환형: 해당 메서드가 실행이 끝나면 호출한 곳으로 되돌아 갈 때 
//						  함께 가지고 가는 결과의 자료형
//						  만약에 결과를 가지고 가지 않으면 void라는 예약어를 사용한다.
//		2) 메서드이름: 변수와 같음, 첫글자 소문자, 두단어 이상일때 단어 첫글자들은 대문자
//					   숫자는 중간이다 뒤에 사용, 공백은 사용 불가
//★중요★ 3) 인자 = 매개변수: 보통은 실행할때 필요한 정보를 외부에서 받기 위한 통로
//						=> 생략 가능하고 여러개 사용 가능
//★고급코딩★ 4) 오버로딩(중복 정의): 메서드 이름이 같으나 인자 수나 인자 자료형이 다르면 다른 메서드로 판단한다
//								즉, 인자의 수나 자료형이 다르면 같은 이름의 메서드가 여러개 존재할 수 있다.
//★중요★ 5) getter() : 호출하는 입장에서 메서드를 이용해서 정보를 가져오는 메서드
...
//	6. static 과 instance: 멤버필드, 멤버메서드에 공통적으로 해당하는 사항
//		1) instance(인스턴스) 필드,메서드: 객체가 생성될때 같이 생성되는 필드와 메서드
